define(['modules/backbone-mozu', 'hyprlive', 'hyprlivecontext', 'modules/jquery-mozu', 'underscore', 'modules/models-customer', 'modules/views-paging', 'modules/editable-view', 'modules/block-ui', 'modules/url-dispatcher', 'modules/intent-emitter', 'modules/get-partial-view', 'modules/on-image-load-error'], function (Backbone, Hypr, HyprLiveContext, $, _, CustomerModels, PagingViews, EditableView, blockUiLoader, UrlDispatcher, IntentEmitter, getPartialView, onImageLoadError) {
    ...
    var AccountSettingsView = EditableView.extend({
        ...
        finishEdit: function () {
            ...
            if (this.beforeEditModel.emailAddress!=='' && this.beforeEditModel.acceptsMarketing) {
                $.ajax({
                    url: '/register/user/?email=' + this.beforeEditModel.emailAddress,
                    method: 'GET',
    ...
    function updateUi(response) {
        ...
        $('html, body').scrollTop($("h1.mz-pagetitle").offset().top - 10);
        setTimeout(function () {
    ...
    $(document).ready(function () {
        ...
        $("ul.mz-scrollnav-list li").click(function (e) {
            ...
            $(".account_content").hide();//fadeOut(500);
            $("#" + activeTab).show();//delay(400).fadeIn(1000);
            //$('html, body').animate({ scrollTop: $("h1.mz-pagetitle").offset().top - 10 }, 700);
            $('html, body').scrollTop($("h1.mz-pagetitle").offset().top - 10);
        ...
        createMstLi();  
        $("body").on("click", ".mz-scrollnav-item", function(e){
            //Tealium Code for search Store starts
            if(HyprLiveContext.locals.themeSettings.tealiumEnabled){
                var utag = window.utag || {};
                var viewItemClicked = $.trim(e.currentTarget.firstElementChild.innerHTML);
                var utagView = {
                    'event_name' : 'my account:'+viewItemClicked
                };
