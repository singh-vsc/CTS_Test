require([
...
], function($, _, bxslider, elevatezoom, blockUiLoader, Hypr, HyprLiveContext, Backbone, CartMonitor, ProductModels, ProductImageViews, RVIModel, FamilyItemView,AdaFunctions, api, async, generalFunctions) {
    ...
    function initslider_mobile() {
        ...
        slider_mobile = $('#productmobile-Carousel').bxSlider({
            ...
            onSlideAfter: function($slideElement, oldIndex, newIndex) {
                ...
                current_zoom_id_added.elevateZoom({ zoomType: "inner", cursor: "crosshair", zoomLevel : 0.75 }).addClass('active');
    ...
    function updateImages(productInitialImages) {
        if (productInitialImages && productInitialImages.mainImage) {
            var mainImage = productInitialImages.mainImage.src + '?maxWidth=' + width_pdp + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey;
            var zoomImage = productInitialImages.mainImage.src + '?maxWidth=' + width_zoom + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey;
            ...
            $('#zoom').elevateZoom({ zoomType: "inner", cursor: "crosshair", zoomLevel : 0.75 });
            try {
        ...
        if (productInitialImages && productInitialImages.thumbImages) {
            ...
            for (var i = 1; i <= productInitialImages.thumbImages.length; i++) {
                $(".mz-productimages-thumbs li:eq(" + (i - 1) + ") .mz-productimages-thumb img")
                    .attr({
                        "src": productInitialImages.thumbImages[i - 1].src + '?maxWidth=' + width_thumb + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey,
                        "data-orig-src": productInitialImages.thumbImages[i - 1].src + '?maxWidth=' + width_pdp + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey,
                        "data-orig-zoom": productInitialImages.thumbImages[i - 1].src + '?maxWidth=' + width_zoom + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey
                    });
            }
            if (slideCount > 4) {
                initSlider();
    ...
    var ProductView = Backbone.MozuView.extend({
                ...
                try {
                    ...
                    var familyItemModelOnready = function() {
                        ...
                        if (typeof product.get('inventoryInfo').onlineStockAvailable !== 'undefined' && product.get('inventoryInfo').onlineStockAvailable === 0 && product.get('inventoryInfo').outOfStockBehavior === "HideProduct") {
                            ...
                            $("#mz-family-container").find("#" + productCode).find(".mz-familylist-item").remove();
        ...
        quantityMinus: function() {
            ...
            var skuID = window.productView.model.get('variationProductCode');
            blockUiLoader.globalLoader();
            this.checkLimitOfSku(skuID, function(response) {
                blockUiLoader.unblockUi();
                if (typeof window.productView.model.get('inventoryInfo').onlineStockAvailable !== "undefined" && window.productView.model.attributes.inventoryInfo.outOfStockBehavior != "AllowBackOrder") {
                    var onlineStock = window.productView.model.get('inventoryInfo').onlineStockAvailable;
                    if (response) {
        ...
        addToCart: _.debounce(function() {
            ...
            if (this.model.get('productType') === Hypr.getThemeSetting('familyProductType')) {
                    ...
                    promises.push((function(callback) {
                        ...
                        familyItem.addToCart().then(function(e) {
                            ...
                            if(HyprLiveContext.locals.themeSettings.tealiumEnabled){
                                var priceObj = (me.model.get('price').get('onSale')) ? me.model.get('price').get('salePrice') : priceObj = me.model.get('price').get('price');
                                var utag = window.utag || {};
                                var utagView = {
                                    'event_name' : 'cart add',
                                    'product_id' : [ me.model.get('productCode') ],
                                    'product_name' : [ me.model.get('content').get('productName') ],
                                    'product_price': [ priceObj ],
                                    'product_quantity' : [ me.model.get('quantity') ],
                                    'product_sku' : [ me.model.get('variationProductCode') ]
                                };
                ...
                async.series(promises, function(err, results) {
                        ...
                        if (!deviceType) {
                            $('html,body').scrollTop($('figure.mz-productimages-main').offset().top);
                        } else {
                            $('html,body').scrollTop($('.mz-product-top-content').offset().top);
                        }
                        blockUiLoader.unblockUi();
                    })
                    /* jshint ignore:end */
                ...
                var skuID = window.productView.model.attributes.variationProductCode;
                this.checkLimitOfSku(skuID, function(response) {
                    if (!response) {
                        me.model.addToCart();
                        //tealium Code starts
        ...
        changeImages: function(colorCode, _updateThumbNails) {
            ...
            var mainImage = imagepath + width_pdp + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey;
            var zoomimagepath = imagepath + width_zoom + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey;
            var _this = this;
                ...
                if (response) {
                    ...
                    if (_updateThumbNails == 'Y') {
                        ...
                        if (deviceType && $('ul.products_list_mobile').length) {
                            ...
                            $(current_zoom_id_added).attr('src', imagepath).data('zoom-image', zoomimagepath).elevateZoom({ zoomType: "inner", cursor: "crosshair", zoomLevel : 0.75 });
                        } else {
                            $('#zoom').removeData('elevateZoom');
        ...
        updateAltImages: function(colorCode) {
            ...
            for (var i = 1; i <= slideCount; i++) {
                $(".mz-productimages-thumbs .products_list li:eq(" + (i - 1) + ") .mz-productimages-thumb img")
                    .attr({
                        "src": imagefilepath + '/' + this.model.attributes.productCode + '_' + colorCode + '_v' + i + '.jpg?maxWidth=' + width_thumb + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey,
                        "data-orig-src": imagefilepath + '/' + this.model.attributes.productCode + '_' + colorCode + '_v' + i + '.jpg?maxWidth=' + width_pdp + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey,
                        "data-orig-zoom": imagefilepath + '/' + this.model.attributes.productCode + '_' + colorCode + '_v' + i + '.jpg?maxWidth=' + width_zoom + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey
                    });
                $(".mz-productimages-thumbs .products_list_mobile li:eq(" + (i - 1) + ") img")
                    .attr({
                        ...
                        "data-orig-src": imagefilepath + '/' + this.model.attributes.productCode + '_' + colorCode + '_v' + i + '.jpg?maxWidth=' + width_pdp + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey,
                        "data-orig-zoom": imagefilepath + '/' + this.model.attributes.productCode + '_' + colorCode + '_v' + i + '.jpg?maxWidth=' + width_zoom + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey,
                        "data-zoom-image": imagefilepath + '/' + this.model.attributes.productCode + '_' + colorCode + '_v' + i + '.jpg?maxWidth=' + width_zoom + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey
                    });
            }
            if (slideCount > 4) {
                initSlider();
        ...
        initialize: function() {
            ...
            if (deviceType && me.model.get('content').get('productImages').length > 1)
                $('#zoom_1').elevateZoom({ zoomType: "inner", cursor: "crosshair", responsive: true, zoomLevel : 0.75 });
            else
                $('#zoom').elevateZoom({ zoomType: "inner", cursor: "crosshair", responsive: true, zoomLevel : 0.75 });
    ...
    $(document).ready(function() {
            ...
            if (cartitem && cartitem.prop('id')) {
                ...
                if (product.get('options') && product.get('options').length && !oneSizeOption) {
                    ...
                    for (var k = 0; k < product.get('options').models.length; k++) {
                        if(optionModels[k].get('values').length>1){
        ...
        var skuID = product.attributes.variationProductCode;
        if(skuID){
            ...
            api.get("cart").then(function(resp) {
                ...
                for (var i = 0; i < cartItems.length; i++) {
                    if (cartItems[i].product.variationProductCode === skuID) {
                        itemQuantityLimit += cartItems[i].quantity;
                    }
                }
                if (require.mozuData('limit') && require.mozuData('limit').stringValue !== "") {
                    var limitperorder = parseInt(JSON.parse(require.mozuData('limit').stringValue)[skuID], 10);
                    if (itemQuantityLimit > limitperorder) {
                        $('[data-mz-validationmessage-for="quantity"]').css('visibility', "visible").text("*Max " + limitperorder + " item(s) are allowed.");
                        $("[data-mz-action='addToCart']").addClass("button_disabled");
                    } else {
