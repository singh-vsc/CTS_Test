define([
    ...
    return {
        ...
        addZoom: function(img, newImgPath) {
            ...
            var zoomImageSrc = currentImageSrc + '?maxWidth=' + widthZoom + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey;
            currentImage.data('zoom-image', currentImageSrc + '?maxWidth=' + widthZoom + '&_mzcb=' +HyprLiveContext.locals.siteContext.generalSettings.cdnCacheBustKey).elevateZoom({ zoomType: "inner", cursor: "crosshair" });
        },
        removeZoom: function() {
            $('body div.zoomContainer').remove();
        ...
        checkCookie: function() {
            ...
            if (breadcrumb && $('body').hasClass('mz-category')) {
                $.cookie('lastCategory', JSON.stringify(breadcrumb), {path: '/'});
            }
            else if ($('body').hasClass('mz-searchresults')) {
                var searchSuffix = (pagecontext.search) ? pagecontext.search.query : '';
                $.cookie('lastCategory', JSON.stringify([{
                    url: pagecontext.url,
                    id: (pagecontext.search) ? pagecontext.search.categoryId : '',
                    name: 'Search Term: ' + searchSuffix
                }]), {path: '/'});
            }
            else {
