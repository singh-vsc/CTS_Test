              ...
              scrubBillingContactId();
                if(self.model.get('id')) {
                        self.model.parent.get('destinations').updateShippingDestinationAsync(self.model).ensure(function () {
                             self.model.trigger('closeDialog');
                               if(!self.model.get('isMultiShipMode')){
                                var ContactId = $("[name='contactId']").find(":selected").data("mzCustomercontactid");
                                var destination = self.model.parent.get('destinations').findWhere({ customerContactId: ContactId });
                                    if (destination) {
                                          destination.saveDestinationAsync().then(function (data) {
                                            self.model.parent.apiSetAllShippingDestinations({
                    ...
                    self.model.parent.get('destinations').saveShippingDestinationAsync(self.model).then(function(data){
                        ...
                        item.updateOrderItemDestination(data.data.id).then(function(){
                            ...
                            if(!self.model.get('isMultiShipMode')){
                                 var target=$("[name='contactId']");
                                 if(target.val()){
                                     self.model.parent.getCheckout().apiSetAllShippingDestinations({
                                        destinationId: target.val()
                                    }).ensure(function () {
                                        //
                                    });
                                 }
