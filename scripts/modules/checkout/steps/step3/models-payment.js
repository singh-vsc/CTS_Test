define([
    ...
    function ($, _, Hypr, Backbone, api, CustomerModels, AddressModels, PaymentMethods, HyprLiveContext, CheckoutStep, ShippingDestinations) {
        ...
        var BillingInfo = CheckoutStep.extend({
            ...
            checkRadial: function(code) {
                var gcServiceUrl = '/status/gift-card/?code=' + code;
            ...
            retrieveDigitalCredit: function (customer, creditCode, me, amountRequested) {
                var self = this;
                return self.checkRadial(creditCode).then(function(resp){
            ...
            getDigitalCredit: function () {
                ...
                if (!creditCode || creditCode.trim() === '') {
                    me.trigger('error', {
                        message: 'No Gift Card Number found'
                    });
                    me.isLoading(false);
                    return;
                }
                ...
                if (existingDigitalCredit && existingDigitalCredit.length > 0) {
                    ...
                    $("input[name=digital-credit-code]:visible").val("");
                    me.unset('digitalCreditCode');
            ...
            addNewContact: function () {
                ...
                this.getOrder().get('dialogContact').get('destinationContact').get('address').set('isBilling', true);
...
*/
            ...
            submit: function () {
                ...
                this.get('billingContact').get('address').set('isBilling', true);
                var val = this.validate();
                this.get('billingContact').get('address').unset('isBilling');
                if(val) {
                    ...
                    if (val['billing-email']) {
                        return false;
                    }
                    if (val['billingContact.email']) {
                ...
                if ($('#billing-email:visible').length && $('#billing-email:visible').val().trim() !== '' && $('input[data-mz-value="acceptsMarketing"]:visible:checked').length) {
                    $.ajax({
                        url: '/register/user/?email=' + $('#billing-email:visible').val().trim(),
                        method: 'GET',
