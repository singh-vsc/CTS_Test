define([
    ...
    var pageSize = Hypr.getThemeSetting('viewAllPageSize');
    ...
    var ProductsListView = Backbone.MozuView.extend({
        ...
        render: function() {
            ...
            _.each(me.model.get('items'),function(itemsRec){
                itemsRec.viewAll = true;
                window._infiniteItems[itemsRec.productCode] = itemsRec;
                //itemsRec.productItem = JSON.stringify(itemsRec);
                //itemsRec.productItem = itemsRec.productItem.replace(/\&quot;\"/g, '\\&quot;\"');
                var view = new ProductListItemView({ model: new Model(itemsRec) });
                var renderedView = view.render().el;
                me.$el.find('#product-list-ul').append(renderedView);
            });
            Backbone.MozuView.prototype.render.apply(this);
            me.$el.find('.cat-page-ul').removeClass('hide');
        },
        addProduct: function(prod) {
            var me = this;
            prod.viewAll = true;
            window._infiniteItems[prod.productCode] = prod;
            var view = new ProductListItemView({ model: new Model(prod) });
            var renderedView = view.render().el;
            me.$el.find('.cat-page-ul').removeClass('hide');
            if($('#product-list-ul').find('.mz-productlist-item[data-mz-product="'+view.model.get('productCode')+'"]').length===0){
